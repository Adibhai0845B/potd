...
r.post("/register", async (req, res) => {
    ...
    const u = await User_1.default.create({ email, passwordHash, username });
    // @ts-ignore
...
r.post("/reset-password", async (req, res) => {
    ...
    const u = await User_1.default.findOne({ resetToken: token }).exec();
    if (!u || !u.resetTokenExpiry || u.resetTokenExpiry < Date.now())
        return res.status(400).json({ error: "Invalid or expired token" });
    u.passwordHash = await bcryptjs_1.default.hash(password, 10);
    u.resetToken = undefined;
    u.resetTokenExpiry = 0;
    await u.save();
